...
export default function SellPage() {
  ...
  const receiveFiatNum = sellAmountNum > 0 ? sellAmountNum * orderPrice : 0;
  const receiveFiat = receiveFiatNum > 0 ? receiveFiatNum.toFixed(2) : "0.00";

  const maxByOrderAmount = availableNum;
  const maxByLimit = orderPrice > 0 ? maxLimitNum / orderPrice : 0;
  ...
  const handleSell = async () => {
    ...
    if (outOfRange) {
      setError(
        `Amount must be between ${minSellByLimit.toFixed(6)} and ${effectiveMaxSell.toFixed(6)} ${orderAsset}`
      );
      return;
    }
    ...
    if (sellAmountNum > availableNum) {
      setError(`Order needs only ${availableNum.toLocaleString()} ${orderAsset}.`);
      return;
    }

    if (receiveFiatNum < minLimitNum || receiveFiatNum > maxLimitNum) {
      setError(`Fiat amount must be between ${minLimitNum.toLocaleString()} and ${maxLimitNum.toLocaleString()} ${orderFiat}`);
      return;
    }
    ...
    const result = await takeOrder(orderId, sellAmountNum);
      ...
      <header className="sticky top-0 z-30 h-16 w-full border-b border-outline-variant/10 bg-surface-container-low px-4">
        <div className="mx-auto flex h-full w-full max-w-3xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-headline text-lg font-bold tracking-tight text-primary">Sell {orderAsset}</h1>
          </div>
        ...
        <section className="relative space-y-6 overflow-hidden rounded-xl bg-surface-container-high p-5 shadow-2xl">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Amount to Sell</label>
              <span className="text-[10px] font-medium text-primary/80">
                Min: {minSellByLimit.toFixed(6)} {orderAsset}
              </span>
            </div>
            <div className="flex items-center rounded-lg border border-outline-variant/15 bg-surface-container-lowest p-4 transition-all focus-within:border-primary/40">
              <input
                ...
                placeholder={`Enter ${orderAsset} amount`}
                type="number"
                value={sellAmount}
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => setSellAmount(String(effectiveMaxSell))}
                  className="text-xs font-bold uppercase tracking-wider text-primary hover:brightness-110"
                >
                  Max
                </button>
                <div className="h-6 w-px bg-outline-variant/30" />
                <span className="font-headline font-bold">{orderAsset}</span>
              </div>
            </div>
            <p className="text-xs text-on-surface-variant">
              Available range: {minSellByLimit.toFixed(6)} - {effectiveMaxSell.toFixed(6)} {orderAsset}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between border-y border-outline-variant/10 py-4">
            ...
            <div className="flex items-center gap-2">
              <span className="font-headline text-2xl font-bold text-primary">{receiveFiat}</span>
              <span className="font-headline font-bold text-on-surface-variant">{orderFiat}</span>
            </div>
          </div>
        ...
        <section className="space-y-4 rounded-xl bg-surface-container-low p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-on-surface-variant">Needs: {availableNum.toLocaleString()} {orderAsset}</p>
            </div>
          </div>
        ...
        <div className="mx-auto w-full max-w-3xl space-y-4">
          <div className="flex items-center justify-between px-2">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Estimated Receive</span>
              <span className="font-headline text-lg font-bold text-on-surface">
                {receiveFiatNum.toLocaleString("en-US", { maximumFractionDigits: 2 })} {orderFiat}
              </span>
            </div>
          ...
          <button
            onClick={handleSell}
            disabled={loading || !orderId || !sellAmount || outOfRange}
