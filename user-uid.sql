...
CREATE SEQUENCE IF NOT EXISTS public.profiles_user_uid_seq START WITH 1000000000;

-- Ensure next generated value starts in Binance-like 10-digit range and stays ahead of existing IDs
SELECT setval(
  'public.profiles_user_uid_seq',
  GREATEST(COALESCE((SELECT MAX(user_uid) FROM public.profiles), 999999999), 999999999) + 1,
  false
);
